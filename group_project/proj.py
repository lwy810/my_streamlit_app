...
print("PASS:", os.getenv("sender_password"))
traceback.print_exc()
...
def main():
    ...
    with col2:
        if st.button("📊 데이터 조회 및 리포트 생성", type="primary"):
            ...
            subject = f"[{selected_sido}] 미세먼지 현황 리포트 - {now_str}"
            body = f"""안녕하세요.
            ...
            try:
                server = smtplib.SMTP("smtp.gmail.com", "587")
                server.starttls()
                server.login("1027ldh@gmail.com", "zicsxivurzvumssh")
                print("✅ SMTP 연결 및 로그인 성공")
                server.quit()
            except Exception as e:
                print("❌ SMTP 연결 또는 로그인 실패:", e )

            # 결과 출력
            if email_result:
                st.success("✅ 이메일이 성공적으로 발송되었습니다!")
            else:
                st.error("❌ 이메일 발송에 실패했습니다. SMTP 설정이나 비밀번호를 확인하세요.")
